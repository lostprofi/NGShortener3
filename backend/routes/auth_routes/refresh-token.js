...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const currentRefreshSession = refreshSessions.find(
      (sess) => sess.refreshToken === refreshToken,
    );
    ...
    await User.findByIdAndUpdate(userId, { $push: { refreshSessions: newRefreshSession } });
  } catch (err) {
    return res.status(500).json({ errors: [{ msg: 'Auth error' }] });
  }
});
